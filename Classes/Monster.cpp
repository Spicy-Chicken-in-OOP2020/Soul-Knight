...
bool Monster::init() {
	...
	/*初始化动画文件*/
	this->_findSprite = Sprite::create("KnightImage.png");
	this->_findSprite->setVisible(false);
	this->addChild(_findSprite, 4);
	this->_findSprite->setPosition(0, 50);
...
void Monster::deadResult() {
	...
	if (poss < 0.5) {
		auto item = Items::createItems(HP_ADD_TAG, this->getPosition());
		dynamic_cast<Layer*>(this->getParent()->getParent())->addChild(item, 2);
	}
	else {
		auto item = Items::createItems(MP_ADD_TAG, this->getPosition());
		dynamic_cast<Layer*>(this->getParent()->getParent())->addChild(item, 2);
	}
}
...
void Monster::update(float dt) {
	...
	if (attackRect->containsPoint(SafeRoomScene::_hero->getPosition())) {
		...
		if (!findEnemy)
		{
			findEnemy = true;
			/*播放动画用的精灵 */
			_findSprite->setVisible(true);

			/*加载出场动画*/
			SpriteFrameCache* spriteFrameCaches = SpriteFrameCache::getInstance();
			spriteFrameCaches->addSpriteFramesWithFile("MonsterFind.plist", "MonsterFind.png");
			...
			for (int i = 1; i <= frameNum; ++i) {//
				frames = spriteFrameCaches->getSpriteFrameByName(StringUtils::format("MonsterFind (%d).png", i));
				log("MonsterFind(%d).png", i);
				frameVecs.pushBack(frames);
			}

			Animation* animation = Animation::createWithSpriteFrames(frameVecs);
			...
			auto callFunc = CallFunc::create([&]() {
				_findSprite->setVisible(false);
				findEnemy = false;
			}
			);

			Action* sequence = Sequence::create(animate, callFunc, nullptr);
			_findSprite->runAction(sequence);
		}
...
void Monster::birthAnimate() {
	...
	auto callFunc = CallFunc::create([&]() {
		...
		_actionSprite->removeFromParentAndCleanup(true);
